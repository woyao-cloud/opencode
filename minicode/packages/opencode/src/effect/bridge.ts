...
export function make(): Effect.Effect<Shape, unknown, unknown> {
  return Effect.gen(function* () {
    ...
    const wrap = <A, E, R>(effect: Effect.Effect<A, E, R>): any => attachWith(effect.pipe(Effect.provide(ctx)) as any, { instance, workspace }) as any
    return {
      ...
      run: <A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E> => Effect.tryPromise(() => Effect.runPromise(wrap(effect))) as any,
    } as Shape
  }) as any
}
