...
export const layer = Layer.effect(Service, Effect.gen(function* () {
  ...
  const messages = Effect.fn("Session.messages")(function* (sessionID: SessionID) {
    const { db } = (yield* InstanceState.get(state)) as State
    return yield* Effect.tryPromise({ try: async () => { const msgRows = await db.select().from(MessageTable).where(eq(MessageTable.session_id, sessionID as string)); const result: Message[] = []; for (const m of msgRows) { const partRows = await db.select().from(PartTable).where(eq(PartTable.message_id, m.id)); result.push({ id: MessageID.make(m.id), sessionID, role: m.role as Message["role"], parts: partRows.map((p: any) => ({ id: PartID.make(p.id), type: p.type as Part["type"], text: p.text ?? undefined, toolName: p.tool_name ?? undefined, toolInput: p.tool_input ? JSON.parse(p.tool_input) : undefined, toolResult: p.tool_result ? JSON.parse(p.tool_result) : undefined })), time: { created: m.time_created } }) } return result }, catch: () => [] as Message[] })
  })
