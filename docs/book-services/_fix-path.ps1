...
foreach ($kv in $map.GetEnumerator()) {
  ...
  $lines = $text -split "`n"
  $pathLine = [char]0x3E + " " + [char]0x6E90 + [char]0x6587 + [char]0x4EF6 + [char]0xFF1A + " `opencode/" + $kv.Value + "`"

  # Remove existing path line (line 2) if present
  if ($lines.Length -gt 1 -and $lines[1] -match '^> .*:') {
    $lines[1] = $pathLine
  } else {
    $newLines = @($lines[0], $pathLine) + $lines[1..($lines.Length - 1)]
    $lines = $newLines
  }
