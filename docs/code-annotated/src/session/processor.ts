/**
 * session/processor - 会话事件处理器：将 LLM 流式事件转换为 Message Part
 *
 * 功能概述：
 * - 管理单次 LLM 调用的完整生命周期（Step Start → Content → Step Finish）
 * - 文本增量合并：将多个 text-delta 合并为连续的 TextPart
 * - 推理内容处理：将 reasoning-delta 合并为 ReasoningPart
 * - 工具调用管理：tool-input-start → tool-input-delta → tool-call 的状态转换
 * - 工具结果收集：completed/error 状态的 ToolPart 更新
 * - 错误处理：Provider 错误、工具执行失败的 Part 化
 * - Token 使用量追踪：input/output/reasoning/cache 的累积统计
 * - 消息快照：在 Step 结束时生成 Assistant Message 的文本快照
 *
 * 核心导出：
 * - Handle：处理器的状态句柄（message、updateToolCall、completeToolCall 等）
 * - Interface/Service/layer：Processor 服务的 Effect 实现
 *
 * 架构位置：会话系统的中间层，位于 LLM 流式输出和 Session 消息存储之间。
 * 上游依赖：Agent、Provider、Bus、Config、Plugin、ToolRegistry、LLM、Truncate
 * 下游消费：session/prompt.ts（runLoop 中调用 processor.create/process）
 */
