/**
 * session/llm - LLM 流式调用封装：将 Effect 生态与 AI SDK 桥接
 *
 * 功能概述：
 * - 封装 AI SDK 的 streamText，提供 Effect/Stream 风格的流式 LLM 调用
 * - 支持多种 Provider 和 Language Model（含 GitLab Workflow 等自定义模型）
 * - System Prompt 组装：合并 Agent Prompt、Provider Prompt、自定义 Prompt
 * - 工具系统集成：从 Plugin 系统收集工具定义，注入权限过滤
 * - 支持 ToolChoice 控制（auto/required/none）
 * - 支持重试、AbortSignal 取消、Otel 追踪
 *
 * 核心导出：
 * - StreamInput/StreamRequest：流式调用输入类型定义
 * - Event：流式事件类型（text-delta、tool-call、step-finish 等）
 * - Interface/Service/layer：LLM 服务的 Effect 实现
 *
 * 架构位置：会话系统的 LLM 网络层，位于 Processor 和 Provider 之间。
 * 上游依赖：Provider、Config、Auth、Plugin、Permission、SystemPrompt
 * 下游消费：session/processor.ts（在 process 方法中消费流式事件）
 */
 ...
 * 检测消息列表中是否包含工具调用内容
 *
 * 用于判断是否需要为 GitHub Copilot 注入占位工具。
 *
 * @param messages - AI SDK ModelMessage 数组
 * @returns 是否包含工具调用/工具结果
 */
