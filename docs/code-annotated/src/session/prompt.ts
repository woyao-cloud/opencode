/**
 * session/prompt - 会话 Prompt 处理核心模块（约2150行，框架最大单文件）
 *
 * 功能概述：
 * - 用户消息的创建与 Part 解析（文本、文件、Agent 引用、MCP 资源、目录）
 * - System Prompt 动态组装：Agent 定义 + 指令文件 + 环境信息 + Skill 注入
 * - 工具注册与执行调度：内置工具 + MCP 工具 → AI SDK Tool 格式转换
 * - Agent 运行循环（runLoop）：多 Step 迭代，工具调用→结果注入→下一轮
 * - Subagent（子任务）调度：Task 工具的完整生命周期管理
 * - Shell 命令执行：用户直接触发的 Shell 工具调用
 * - 斜杠命令（Command）处理：模板解析、参数替换、Subagent 路由
 * - 自动标题生成：首个用户消息后异步生成会话标题
 * - Plan 模式：Plan/Build Agent 切换、Plan 文件管理、结构化输出
 * - 上下文溢出检测 → 自动 Compaction 触发
 * - 事件发布：Agent 切换、模型切换、Prompted、Synthetic 等事件
 *
 * 核心导出：
 * - Interface / Service / layer / defaultLayer：Prompt 服务的 Effect 实现
 * - PromptInput / LoopInput / ShellInput / CommandInput：输入 Schema
 * - createStructuredOutputTool：结构化输出工具工厂（JSON Schema 模式）
 *
 * 架构位置：opencode 框架最核心模块，位于会话系统顶层。
 * 上游依赖：Agent、Provider、Session、SessionCompaction、SessionProcessor、
 *   SystemPrompt、Instruction、ToolRegistry、MCP、LSP、Plugin、Permission、
 *   Command、Config、Bus、Truncate、Image、LLM、Reference 等几乎所有核心模块
 * 下游消费：CLI run 命令、ACP Agent、Server API 处理器
 */
 ...
 * @internal 导出用于测试
 *
 * 包装 AI SDK 的 tool()，移除 $schema 属性，在 execute 中调用 onSuccess 回调。
 * 适用于 JSON Schema 格式要求的场景（如 plan mode 的结构化输出）。
 *
 * @param input.schema - JSON Schema 定义
 * @param input.onSuccess - 输出成功回调
