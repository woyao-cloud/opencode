/**
 * session/projectors-next - 会话事件投影器（Next 版本）：将 V2 事件持久化到 session_message 表
 *
 * 功能概述：
 * - 监听 SessionEvent 的各类事件（AgentSwitched/ModelSwitched/Prompted/Synthetic 等）
 * - 将 V2 事件通过 SessionMessageUpdater 写入 session_message 表
 * - 支持流式事件的增量更新（Text.Delta/Tool.Input.Delta/Reasoning.Delta 无操作）
 * - 会话级变更同步到 SessionTable（agent/model 更新）
 *
 * 核心导出：
 * - 默认导出 SyncEvent.project 数组
 *
 * 架构位置：会话系统的事件持久化层。
 * 上游依赖：SessionMessageUpdater、SessionEvent、SyncEvent、EventV2Bridge
 * 下游消费：session/projectors.ts（导入并合并到主投影器列表）
 */
