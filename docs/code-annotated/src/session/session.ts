/**
 * session/session - 会话 CRUD 服务：创建、查询、更新、删除会话
 *
 * 功能概述：
 * - 会话创建：支持普通创建和从已有会话 fork 子会话
 * - 会话查询：按项目/目录/工作区/根会话等条件列表查询，支持分页和搜索
 * - 会话更新：标题、归档、权限、回滚状态、摘要、差异等字段更新
 * - 消息管理：消息/Part 的增删改查，支持分页翻页和按条件查找
 * - 会话删除：级联删除子会话和关联的背景任务
 * - Token 用量计算：根据 Provider 定价计算成本和 Token 统计
 * - 事件同步：所有变更通过 SyncEvent 持久化到数据库
 *
 * 核心导出：
 * - Info/Schema：会话信息的完整 Schema 定义和 TypeScript 类型
 * - Event：会话事件的 SyncEvent 定义（Created/Updated/Deleted/Diff/Error）
 * - Interface/Service/layer/defaultLayer：Session 服务的 Effect 实现
 * - 工具函数：fromRow/toRow/getUsage/isDefaultTitle/listGlobal
 *
 * 架构位置：会话系统的核心服务层，所有会话操作的入口。
 * 上游依赖：BackgroundJob、Bus、Storage、SyncEvent、MessageV2、Snapshot
 * 下游消费：session/prompt.ts、session/compaction.ts、session/revert.ts、CLI/TUI
 */
...
/**
 * 将会话 Info 对象转换为数据库行格式
 *
 * @param info - 会话 Info 对象
 * @returns 数据库行对象
 */
 ...
 * 计算会话的 Plan 文件路径
 *
 * 基于 VCS 状态选择路径：有 VCS 时放在项目 .opencode/plans 目录，
 * 无 VCS 时放在全局数据目录下的 plans 目录。
 *
 * @param input - 会话信息（slug、创建时间）
 * @param instance - 实例上下文
 * @returns Plan 文件的绝对路径
 */
 ...
 * 计算 Token 用量和成本
 *
 * 根据 Provider 定价按阶梯计算输入/输出/缓存/推理 Token 的成本。
 * AI SDK v6 已将缓存 Token 包含在 inputTokens 中，需减去缓存 Token
 * 以分别计算非缓存输入和缓存的成本。
 * 推理 Token 按输出 Token 同费率计费。
 *
 * @param input.model - Provider 模型信息（含定价）
 * @param input.usage - AI SDK 返回的 Token 用量
 * @param input.metadata - Provider 元数据（缓存创建 Token 等）
 * @returns 成本金额和 Token 用量详情
 */
 ...
 * 全局会话列表迭代器（跨项目查询）
 *
 * 支持按目录、根会话、更新时间范围、搜索关键字、归档状态等条件过滤。
 * 同时会查询关联的项目信息（名称、工作目录）一并返回。
 *
 * @param input - 查询条件（directory/roots/start/cursor/search/limit/archived）
 * @yields GlobalInfo 对象（含项目信息）
 */
