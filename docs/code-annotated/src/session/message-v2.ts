/**
 * session/message-v2 - V2 协议消息与 Part 的 Schema 定义（核心类型文件）
 *
 * 功能概述：
 * - 定义所有 Part 类型：TextPart、FilePart、ToolPart、ReasoningPart、
 *   StepStartPart、StepFinishPart、SnapshotPart、PatchPart、AgentPart、
 *   RetryPart、CompactionPart、SubtaskPart
 * - 定义 ToolState 联合类型：Pending → Running → Completed → Error 状态机
 * - 定义 Assistant Message 的完整结构（parts、tokens、cost、error、finish）
 * - 定义 User Message 结构（role、agent、model、system、format）
 * - 提供消息过滤/转换工具：filterCompactedEffect、toModelMessagesEffect、latest
 * - 定义输入类型：TextPartInput、FilePartInput、AgentPartInput、SubtaskPartInput
 * - 定义错误类型：StructuredOutputError 等
 * - 截断工具输出：truncateToolOutput
 *
 * 核心导出：
 * - 所有 Part/Message 的 Schema 类和 TypeScript 类型
 * - 工具函数：filterCompactedEffect、toModelMessagesEffect、latest、fromError
 * - 输入类型：TextPartInput、FilePartInput、AgentPartInput、SubtaskPartInput
 *
 * 架构位置：会话系统的数据类型基础层，被几乎所有 session/ 和 tool/ 模块依赖。
 * 上游依赖：session/schema（ID 类型）、core/session-event、core/model、bus/bus-event
 * 下游消费：session/prompt.ts、session/processor.ts、tool/*、CLI/TUI 渲染层
 */
 ...
 * 游标编码/解码工具
 *
 * 用于消息列表的分页翻页，将 Cursor（messageID + timestamp）编码为 base64url 字符串。
 */
export const cursor = {
  encode(input: Cursor) {
...
/**
 * 将消息流转换为 AI SDK ModelMessage 格式
 *
 * 处理消息的历史顺序、工具调用状态、推理内容、媒体文件等。
 * 对 OpenAI 兼容 API，将工具结果中的媒体文件提取为用户消息。
 * 处理 Anthropic 有符号推理的边界 case（空文本分隔符替换为空格）。
 *
 * @param input - 消息列表
 * @param model - Provider 模型信息
 * @param options.stripMedia - 是否去除媒体文件（用于压缩）
 * @param options.toolOutputMaxChars - 工具输出截断长度
...
/**
 * 将消息流转换为 AI SDK ModelMessage 格式（同步版本）
 *
 * 同步包装 toModelMessagesEffect，用于非 Effect 上下文中。
 *
 * @param input - 消息列表
 * @param model - Provider 模型信息
 * @param options - 可选配置（stripMedia、toolOutputMaxChars）
 * @returns ModelMessage 数组的 Promise
 */
 ...
 * @param input.limit - 每页数量
 * @param input.before - 翻页游标（base64url 编码的 Cursor）
 * @returns 消息列表和翻页信息
 */
 ...
 * 过滤已压缩的消息流，按模型消费顺序重排
 *
 * 压缩后的消息会按 [compaction-user, summary, ...retained tail..., continue-user]
 * 的顺序重新排列，以便模型正确理解上下文。
 *
 * @param msgs - 原始消息迭代器
 * @returns 重排后的消息数组
...
/**
 * 从重排后的消息流中提取最新的用户/助手/已完成消息和待处理任务
 *
 * 使用 MessageID（单调递增）确定最新消息，而不是数组位置。
 * 因为 filterCompacted 会重排消息顺序。
 * 待处理任务是指比最新已完成助手消息更新的 Compaction/Subtask Part。
 *
 * @param msgs - 重排后的消息数组
 * @returns 最新消息和待处理任务
 */
...
/**
 * 将各种类型的错误转换为标准化的 Assistant 错误对象
 *
 * 处理多种错误类型：AbortError、OutputLengthError、LoadAPIKeyError、
 * ECONNRESET、ZlibError、APICallError（含 context overflow 解析）、
 * 通用 Error、以及 Provider 流式错误。
 *
 * @param e - 原始错误
 * @param ctx.providerID - Provider ID
 * @param ctx.aborted - 是否被用户中断
 * @returns 标准化的错误对象
 */
