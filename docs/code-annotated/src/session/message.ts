/**
 * session/message - V1 消息 Schema 定义（兼容层）
 *
 * 功能概述：
 * - 定义 ToolCall/ToolPartialCall/ToolResult 等 V1 工具调用类型
 * - 定义 TextPart/ReasoningPart/ToolInvocationPart/SourceUrlPart/FilePart 等 Part 类型
 * - 定义完整 Message 结构（id、role、parts、metadata）
 * - 提供 ToolInvocation 联合类型的鉴别器
 *
 * 核心导出：
 * - 所有 Part/Message 的 Schema 类和 TypeScript 类型
 *
 * 架构位置：V1 消息类型定义层，V2 迁移的兼容适配。
 * 上游依赖：session/schema、session/message-error、provider/schema
 * 下游消费：session/message-v2.ts（V2 类型引用 V1 的 ToolInvocation 等）
 */
