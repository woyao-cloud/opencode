/**
 * session/projectors - 会话事件投影器：将 SyncEvent 持久化到数据库
 *
 * 功能概述：
 * - 监听 Session 事件（Created/Updated/Deleted）并写入 SessionTable
 * - 监听 MessageV2 事件（Updated/Removed/PartRemoved/PartUpdated）并写入 MessageTable/PartTable
 * - Token 用量同步：step-finish Part 的 cost/tokens 实时累加到 SessionTable
 * - 外键冲突处理：延迟消息/Part 更新时自动忽略
 * - 导入并合并 projectors-next 的事件处理
 *
 * 核心导出：
 * - toPartialRow：将 Session.Info 部分更新转换为数据库行格式
 * - 默认导出 SyncEvent.project 处理函数数组
 *
 * 架构位置：会话系统的 CQRS 投影层，将事件流持久化到 SQLite。
 * 上游依赖：SyncEvent、Session、MessageV2、projectors-next
 * 下游消费：SyncEvent 系统自动调度
 */
...
/**
 * 将会话 Info 的部分更新对象转换为数据库行格式
 *
 * 使用 grab 函数从嵌套对象中安全提取字段值，
 * 过滤掉 undefined 字段，只返回有值的部分。
 * 注意：传递 undefined 会抛出错误，传递 null 才能清除字段。
 *
 * @param info - 会话 Info 的部分更新对象
 * @returns 数据库行格式的部分更新对象
 */
