/**
 * worktree/index - Git Worktree 服务：管理 Git 工作树（worktree）的创建和删除
 *
 * 功能概述：
 * - 创建基于 Git 分支的隔离工作树
 * - 管理工作树的清理和删除
 ...
 *
 * 核心导出：
 * - Service：工作树服务 Context 类
 * - layer：工作树服务 Effect Layer
 *
 * 架构位置：Worktree 模块服务层，提供 Git 工作树的隔离开发环境。
 * 上游依赖：project/instance-layer（实例 Layer）、project/instance-store（实例存储）
 */
