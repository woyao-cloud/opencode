/**
 * git/index - Git 服务：封装 Git 命令行操作，提供 Effect 式 Git 接口
 *
 * 功能概述：
 * - 封装 Git 命令行调用（状态、差异、日志、分支等）
 * - 统一的 Git 配置参数（autocrlf、fsmonitor 等）
 * - 提供 Effect 风格的 Git 操作接口
 *
 * 核心导出：
 * - Service：Git 服务 Context 类
 * - layer：Git 服务 Effect Layer
 *
 * 架构位置：Git 模块服务层，被 project/vcs、storage/storage 等模块使用。
 * 上游依赖：effect（Effect 系统）
 */
