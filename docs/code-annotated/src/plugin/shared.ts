/**
 * plugin/shared - 插件共享工具：插件解析、版本兼容性检查和源识别
 *
 * 功能概述：
 * - 解析插件说明符（npm 包名、文件路径）
 * - 检查插件版本兼容性
 ...
 *
 * 核心导出：
 * - pluginSource：确定插件来源类型
 * - checkPluginCompatibility：版本兼容性检查
 * - resolvePluginTarget：解析插件目标路径
 *
 * 架构位置：Plugin 模块工具层，被 plugin/loader.ts 和 plugin/meta.ts 使用。
 */
