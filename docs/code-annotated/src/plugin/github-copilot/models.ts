/**
 * plugin/github-copilot/models - Copilot 模型 Schema：定义 Copilot 可用模型的类型和 Schema
 *
 * 功能概述：
 * - 定义 Copilot 模型列表的 Effect Schema
 * - 包含模型 ID、名称、版本、策略等信息
 *
 * 核心导出：
 * - schema：Copilot 模型列表响应 Schema
 *
 * 架构位置：Plugin 模块数据层，被 copilot.ts 用于解析 API 响应。
 */
