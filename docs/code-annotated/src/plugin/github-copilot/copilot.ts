/**
 * plugin/github-copilot/copilot - GitHub Copilot 认证插件：处理 GitHub Copilot 的 OAuth 认证和模型访问
 *
 * 功能概述：
 * - 通过 GitHub OAuth 流程获取 Copilot 访问令牌
 * - 管理令牌的刷新和持久化
 * - 获取 Copilot 可用模型列表
 *
 * 核心导出：
 * - CopilotAuthPlugin：Copilot 认证插件工厂函数
 *
 * 架构位置：Plugin 模块认证插件层，被 plugin/index.ts 加载。
 * 上游依赖：plugin/github-copilot/models（模型 Schema）
 */
