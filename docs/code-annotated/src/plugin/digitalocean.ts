/**
 * plugin/digitalocean - DigitalOcean 认证插件：处理 DigitalOcean 推理 API 的 OAuth 认证
 *
 * 功能概述：
 * - 通过 OAuth 2.0 流程获取 DigitalOcean API 令牌
 * - 启动本地 HTTP 服务器处理 OAuth 回调
 * - 管理令牌刷新和持久化
 *
 * 核心导出：
 * - DigitalOceanAuthPlugin：DigitalOcean 认证插件工厂函数
 *
