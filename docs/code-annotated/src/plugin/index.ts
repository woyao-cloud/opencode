/**
 * plugin/index - 插件服务层：管理 opencode 插件的加载、注册和生命周期
 *
 * 功能概述：
 * - 加载和初始化外部插件（认证、工作区适配器等）
 * - 管理插件的安装、更新和卸载
 * - 集成总线事件通知插件状态变更
 *
 * 核心导出：
 * - Service：插件服务 Context 类
 * - layer：插件服务 Effect Layer
 *
 * 架构位置：Plugin 模块服务层，是 opencode 插件系统的核心入口。
 * 上游依赖：plugin/codex、plugin/digitalocean 等认证插件
 */
import type {
