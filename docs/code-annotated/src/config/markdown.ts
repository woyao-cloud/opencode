/**
 * [config/markdown] - Markdown 文件配置解析模块
 *
 * 功能概述：
 * - 从 Markdown 文件中提取文件引用（@path）和 shell 命令（!`cmd`）
 * - 解析 YAML 前置元数据（frontmatter），含非标准 YAML 的回退处理
 *
 * 核心导出：
 * - files()：提取 Markdown 模板中的文件引用
 * - shell()：提取 Markdown 模板中的 shell 命令
 * - FrontmatterError：前置元数据解析错误
 *
 * 架构位置：上游被 config/command.ts、config/agent.ts 等引用
 */
