/**
 * [config/permission] - 权限配置模块
 *
 * 功能概述：
 * - 定义权限规则的配置 Schema（ask/allow/deny）
 * - 支持按操作（read/edit/glob/run 等）配置不同权限
 * - 支持 Action 简写和对象形式之间的规范化转换
 *
 * 核心导出：
 * - Action：权限动作 Schema（"ask" | "allow" | "deny"）
 * - Rule：权限规则 Schema（Action 或对象映射）
 * - Info：权限配置顶层 Schema
 *
