/**
 * [config/agent] - Agent 配置模块：从 Markdown 文件加载 Agent 定义
 *
 * 功能概述：
 * - 定义 Agent 配置 Schema（模型、变体、提示词、权限、步骤数等）
 * - 从 {agent,agents}/**\/*.md 文件加载 Agent 配置
 * - 从 {mode,modes}/*.md 文件加载 Mode 配置
 * - 后处理标准化：未知键 → options、tools → permission、steps ← maxSteps
 *
 * 核心导出：
 * - Info：Agent 配置 Schema
 * - load：从目录加载所有 Agent 配置
 * - loadMode：从目录加载所有 Mode 配置
 *
