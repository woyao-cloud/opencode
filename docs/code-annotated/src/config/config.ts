/**
 * [config/config] - 配置系统核心模块：配置加载、合并、持久化
 *
 * 功能概述：
 * - 定义完整的配置 Schema（Info），覆盖所有可配置项（shell、logLevel、provider、mcp、agent 等）
 * - 实现多层配置加载和合并：全局配置 → 远程配置 → 项目配置 → 环境变量 → 托管配置
 * - 支持多种配置源：文件系统、环境变量、远程 HTTP、macOS MDM 托管配置
 * - 提供配置更新、全局配置修改、JSONC 编辑支持
 * - 管理插件来源追踪（plugin_origins），处理去重和范围判定
 *
 * 核心导出：
 * - Service / layer：Effect 配置服务层
 * - Info：配置 Schema 类型
 * - ConfigDirectoryTypoError：目录拼写错误类型
 *
 * 架构位置：位于 config 层核心，被几乎所有模块依赖
 */
