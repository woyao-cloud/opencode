/**
 * snapshot/index - 快照服务：管理项目文件的快照和差异补丁
 *
 * 功能概述：
 * - 创建文件快照（记录文件哈希和内容）
 * - 生成文件差异补丁
 ...
 *
 * 核心导出：
 * - Service：快照服务 Context 类
 * - layer：快照服务 Effect Layer
 * - Patch：文件补丁 Schema
 *
 * 架构位置：Snapshot 模块服务层，为 project/vcs 提供文件快照能力。
 * 上游依赖：effect（Effect 系统）、git（Git 服务）
 */
