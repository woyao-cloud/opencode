/**
 * share/share-next - 下一代分享实现：基于 Effect 的会话分享核心逻辑
 *
 * 功能概述：
 * - 使用 Effect 系统实现会话分享
 * - 管理分享令牌和 URL 生成
 * - 集成 Provider 和 Account 系统
 *
 * 核心导出：
 * - Service：下一代分享服务 Context 类
 * - layer：下一代分享 Effect Layer
 *
 * 架构位置：Share 模块核心实现层，提供分享功能的具体逻辑。
 * 上游依赖：account/account（账户系统）、session/session（会话管理）
 */
