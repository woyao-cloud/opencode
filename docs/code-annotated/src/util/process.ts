/**
 * util/process - 进程管理与命令执行工具
 *
 * 功能概述：
 * - 提供跨平台的子进程创建、管理和通信功能
 * - 支持超时控制、中止信号、进程树终止等高级特性
 *
 * 核心导出：
 * - spawn：创建子进程，返回 Child 对象
 * - run：执行命令并等待结果（返回 stdout/stderr）
 * - text：执行命令并返回文本输出
 * - lines：执行命令并返回行数组
 * - stop：安全终止进程（支持 Windows 进程树）
 * - RunFailedError：命令执行失败的自定义错误
 *
 * 架构位置：通用工具层，依赖 error 模块和 cross-spawn 库
 */
