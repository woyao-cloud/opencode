/**
 * util/filesystem - 文件系统操作工具集
 *
 * 功能概述：
 * - 提供文件读写、路径解析、目录遍历等文件系统操作的封装
 * - 支持 Windows 路径规范化、跨平台兼容
 *
 * 核心导出：
 * - exists / isDir / stat：文件状态查询
 * - readText / readJson / readBytes / readArrayBuffer：文件读取
 * - write / writeJson / writeStream：文件写入
 * - resolve / normalizePath / windowsPath：路径解析与规范化
 * - findUp / up / globUp：向上查找文件
 * - contains / overlaps：路径关系判断
 *
 * 架构位置：通用工具层，依赖 Glob 模块和 Node.js fs/path 模块
 */
