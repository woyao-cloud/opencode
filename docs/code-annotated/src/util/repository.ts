/**
 * util/repository - Git 仓库引用解析工具
 *
 * 功能概述：
 * - 解析各种格式的 Git 仓库引用（HTTPS、SSH、SCP、GitHub 简写等）
 * - 提供仓库引用比较和缓存路径生成功能
 *
 * 核心导出：
 * - parseRepositoryReference：解析仓库引用字符串为结构化对象
 * - parseRemoteRepositoryReference：解析并验证远程仓库引用
 * - parseGitHubRemote：提取 GitHub 仓库的 owner/repo
 * - validateRepositoryBranch：验证分支名格式
 * - sameRepositoryReference：比较两个仓库引用是否相同
 * - repositoryCachePath：生成仓库缓存路径
 *
 * 架构位置：通用工具层，依赖 Global 模块获取缓存目录
 */
