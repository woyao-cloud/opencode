/**
 * storage/db - 数据库服务：SQLite 数据库管理和操作入口
 *
 * 功能概述：
 * - 初始化和管理 SQLite 数据库连接
 * - 执行数据库迁移
 ...
 *
 * 核心导出：
 * - Database：drizzle ORM 数据库类型
 * - 导出所有 drizzle-orm 工具函数
 *
 * 架构位置：Storage 模块核心，提供统一的数据库访问接口。
 * 上游依赖：storage/db.bun 或 storage/db.node（平台适配）
 */
