/**
 * storage/storage - 文件存储服务：基于 Git 的文件版本管理和存储
 *
 * 功能概述：
 * - 使用 Git 作为底层文件存储
 * - 管理文件版本快照和迁移
 * - 提供文件存储的 Effect Layer
 *
 * 核心导出：
 * - Service：存储服务 Context 类
 * - layer：存储服务 Effect Layer
 *
 * 架构位置：Storage 模块服务层，提供文件级别的版本化存储能力。
 * 上游依赖：git/index（Git 服务）
 */
