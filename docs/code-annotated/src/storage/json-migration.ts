/**
 * storage/json-migration - JSON 数据迁移：将旧版 JSON 存储迁移到 SQLite 数据库
 *
 * 功能概述：
 * - 从旧版 JSON 文件中读取项目、会话和权限数据
 * - 将数据写入 SQLite 数据库
 * - 支持进度报告和增量迁移
 *
 * 核心导出：
 ...
 * - Progress：迁移进度类型
 *
 * 架构位置：Storage 模块迁移层，用于从旧存储格式过渡到 SQLite。
 * 上游依赖：storage/db（数据库连接）、project/project.sql（目标表）
 */
