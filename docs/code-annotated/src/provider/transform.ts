/**
 * [provider/transform] - Provider 消息转换和模型变体配置
 *
 * 功能概述：
 * - 提供消息标准化函数，针对不同 LLM 提供商（Anthropic、Bedrock、Mistral、DeepSeek 等）做格式适配
 * - 管理模型的 reasoning_effort 变体配置（不同提供商支持的推理努力级别不同）
 * - 处理消息缓存、不支持的模态内容过滤、providerOptions 键名重映射
 * - 提供模型选项（temperature、topP、topK）和输出 Token 上限的计算
 * - 处理工具调用 Schema 的提供商特定清理（如 Moonshot、Gemini）
 *
 * 核心导出：
 * - message：消息处理主函数（标准化 + 缓存 + 键名重映射）
 * - variants：根据模型能力生成推理努力变体配置
 * - options：生成模型调用的选项参数
 * - schema：清理工具调用 JSON Schema 以适配不同提供商
 * - temperature / topP / topK / maxOutputTokens：模型参数计算函数
 *
 * 架构位置：位于 provider 层，被 provider.ts 调用，用于准备发送给 AI SDK 的消息和选项
 */
...
/**
 * 输出 Token 上限常量（32000）
 * 用于限制模型单次回复的最大输出 Token 数
 */
export const OUTPUT_TOKEN_MAX = 32_000

/**
 * 清理字符串中的孤立代理对（surrogate pairs）
 *
 * 某些 LLM 提供商（如 Anthropic）会拒绝包含无效 Unicode 代理对的请求。
 * 此函数将孤立的低位/高位代理字符替换为 U+FFFD（替换字符）。
 *
 * @param content - 要清理的原始字符串
 * @returns 清理后的字符串
 */
...
/**
 * 将 npm 包名映射到 AI SDK 期望的 providerOptions 键名
 *
 * 不同的 AI SDK 包使用不同的键名来读取 providerOptions（如 "anthropic"、"openai"、"bedrock" 等）。
 * 此映射确保写入的键名与 SDK 读取的键名一致。
 *
 * @param npm - npm 包名
 * @returns AI SDK 中对应的 providerOptions 键名，未知包返回 undefined
 */
...
/**
 * 标准化消息列表以适应不同 LLM 提供商的要求
 *
 * 此函数处理各提供商特有的消息格式要求：
 * - Anthropic：拒绝空内容消息、要求 tool_use 和 tool_result 严格交替
 * - Bedrock：类似 Anthropic 的空内容过滤
 * - Claude：清理 toolCallId（仅允许字母数字和下划线）
 * - Mistral：限制 toolCallId 为 9 位字母数字
 * - DeepSeek：要求所有 assistant 消息包含 reasoning 字段
 * - OpenAI 兼容：将 reasoning 内容移到 providerOptions 中
 *
 * @param msgs - AI SDK 格式的消息列表
 * @param model - 目标模型信息
 * @param _options - 额外选项（当前未使用）
 * @returns 标准化后的消息列表
 */
 ...
 * 为消息添加缓存控制标记
 *
 * 对系统提示（前 2 条）和最近消息（后 2 条）应用缓存标记，以利用
 * Anthropic / Bedrock / OpenRouter 等提供商的提示缓存功能。
 * 支持消息级别和内容部分级别的缓存标记。
 *
 * @param msgs - 消息列表
 * @param model - 模型信息（用于判断缓存策略）
 * @returns 添加了缓存标记的消息列表
 */
 ...
 * 过滤模型不支持的模态内容
 *
 * 检查用户消息中的文件/图像附件是否被当前模型支持。
 * 如果不支持，将附件替换为错误提示文本，告知用户该模型不支持此类输入。
 *
 * @param msgs - 消息列表
 * @param model - 模型信息（包含能力声明）
 * @returns 过滤后的消息列表
 */
...
/**
 * 消息处理主函数：标准化 + 缓存 + providerOptions 键名重映射
 *
 * 处理流程：
 * 1. 过滤模型不支持的模态内容
 * 2. 执行提供商特定的消息标准化
 * 3. 对支持缓存的模型应用缓存标记
 * 4. 重映射 providerOptions 键名（从 providerID 到 SDK 期望的键名）
 *
 * @param msgs - AI SDK 格式的消息列表
 * @param model - 目标模型信息
 * @param options - 额外选项
...
/**
 * 根据模型 ID 计算推荐的 temperature 参数
 *
 * 不同模型有各自推荐的 temperature 范围：
 * - Qwen：0.55
 * - Claude：undefined（让 SDK 使用默认值）
 * - Gemini / GLM / MiniMax：1.0
 * - Kimi K2：0.6（特定变体为 1.0）
 *
 * @param model - 模型信息
 * @returns 推荐的 temperature 值，undefined 表示使用 SDK 默认值
 */
...
/**
 * 根据模型 ID 计算推荐的 topP 参数
 *
 * @param model - 模型信息
 * @returns 推荐的 topP 值，undefined 表示使用 SDK 默认值
 */
...
/**
 * 根据模型 ID 计算推荐的 topK 参数
 *
 * @param model - 模型信息
 * @returns 推荐的 topK 值，undefined 表示使用 SDK 默认值
 */
...
/**
 * 根据模型能力和提供商生成推理努力（reasoning_effort）变体配置
 *
 * 不同 AI SDK 包使用不同的字段名和格式来控制推理努力级别：
 * - @ai-sdk/openai：reasoningEffort
 * - @ai-sdk/anthropic：thinking.type / thinking.budgetTokens
 * - @ai-sdk/amazon-bedrock：reasoningConfig
 * - @ai-sdk/google：thinkingConfig
 * - @ai-sdk/mistral：reasoningEffort
 * - @ai-sdk/groq：reasoningEffort
 * - ai-gateway-provider：reasoningEffort
 *
 * 支持的推理努力级别因模型而异（如 GPT-5 系列支持 "none" ~ "xhigh"）。
 *
 * @param model - 模型信息
 * @returns 变体配置映射表
 */
 ...
 *
 * 处理各提供商的特定选项配置：
 * - OpenAI / Azure：关闭 store、设置 promptCacheKey
 * - OpenRouter：启用 usage 追踪
 * - Google / Vertex：配置 thinkingConfig
 * - Gateway：配置缓存策略
 * - GPT-5 系列：设置 reasoningEffort、reasoningSummary、textVerbosity
 * - Kimi / ZhipuAI：启用 thinking 模式
 *
 * @param input - 包含模型、会话 ID 和额外 providerOptions 的输入
 * @returns 模型调用选项对象
 */
 ...
 * 生成"小模型"的选项配置
 *
 * 用于快速任务（如标题生成），选择模型的第一个变体作为"小模型"选项。
 * 某些提供商需要特殊处理（如 OpenAI 关闭 store、Venice 禁用思考）。
 *
 * @param model - 模型信息
 * @returns 小模型选项配置
 */
...
/**
 * 生成 providerOptions，处理键名映射和 Gateway 特殊逻辑
 *
 * Gateway 提供商的 providerOptions 需要拆分为两个命名空间：
 * - gateway：Gateway 原生控制（路由、缓存等）
 * - <upstream slug>：上游提供商特定的模型选项
 *
 * Azure 需要同时传递 openai 和 azure 两个命名空间。
 *
 * @param model - 模型信息
 * @param options - 选项对象
 * @returns providerOptions 对象
 */
 ...
 * 计算模型的最大输出 Token 数
 *
 * 取模型限制和配置上限的较小值，避免超出模型的输出限制。
 *
 * @param model - 模型信息
 * @param outputTokenMax - 配置的输出 Token 上限（默认 32000）
 * @returns 实际可用的最大输出 Token 数
 */
...
/**
 * 清理工具调用的 JSON Schema 以适应不同提供商的限制
 *
 * 处理以下提供商的 Schema 兼容性问题：
 * - Moonshot/Kimi：展开 $ref、不支持元组式 items
 * - Google/Gemini：将整数枚举转为字符串、移除非对象类型的 properties/required
 *
 * @param model - 模型信息
 * @param schema - 原始 JSON Schema
 * @returns 清理后的 JSON Schema
 */
