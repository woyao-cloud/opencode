/**
 * [provider/auth] - Provider 认证模块：管理 OAuth 和 API Key 认证流程
 *
 * 功能概述：
 * - 提供 provider 的认证方法注册和调用接口（OAuth / API Key）
 * - 处理 OAuth 授权流程：authorize → callback 的完整生命周期
 * - 支持通过插件钩子（plugin hooks）注入自定义认证逻辑
 * - 提供交互式认证提示（文本输入/选择框）的 Schema 定义
 *
 * 核心导出：
 * - Service / layer：Effect 认证服务层
 * - Method / Methods / Authorization：认证 Schema 定义
 * - AuthorizeInput / CallbackInput：输入参数 Schema
 * - OauthMissing / OauthCodeMissing / OauthCallbackFailed / ValidationFailed：错误类型
 *
 * 架构位置：位于 provider 层，被 provider.ts 调用，依赖 Auth 和 Plugin 服务
 */
