/**
 * [provider/provider] - Provider 核心模块：模型注册、发现、SDK 加载与语言模型实例化
 *
 * 功能概述：
 * - 管理 LLM 提供商的注册生命周期：从 models.dev 目录加载 → 环境变量/API Key 注入 → 自定义配置合并 → 模型过滤
 * - 提供统一的 Provider 接口（list / getProvider / getModel / getLanguage / defaultModel 等）
 * - 支持动态 SDK 加载：内置 BUNDLED_PROVIDERS 或按需从 npm 安装第三方 SDK
 * - 处理各提供商的特殊配置逻辑（Azure 资源名、Bedrock 区域前缀、Cloudflare AI Gateway 等）
 * - 提供模型搜索建议（模糊匹配 + 关键词评分）
 *
 * 核心导出：
 * - Service / layer：Effect 服务层定义
 * - Model / Info / ListResult：Schema 定义
 * - ProviderID / ModelID：品牌类型
 * - ModelNotFoundError / InitError：错误类型
 * - sort / parseModel / defaultModelIDs / toPublicInfo：工具函数
 *
 * 架构位置：provider 层的核心模块，依赖 Config / Auth / Env / Plugin / ModelsDev 等服务
 */
import os from "os"
...
/**
 * 判断是否应使用 Copilot Responses API
 * GPT-5 及以上版本使用 Responses API，但 GPT-5-mini 除外
 */
...
/**
 * 包装 SSE 响应流，添加单次读取超时
 * 每个 chunk 的读取操作都有独立的超时控制，超时后会中止整个流
 */
...
/**
 * 将 Provider 信息转换为可公开的版本（移除函数、Symbol 等不可序列化的字段）
 */
...
/**
 * 模型排序函数
 * 优先级顺序：GPT-5 > Claude Sonnet 4 > big-pickle > Gemini 3 Pro
 * 同优先级内，"latest" 版本排在前面
 */
...
/**
 * 解析 "providerID/modelID" 格式的模型字符串
 * 例如 "anthropic/claude-sonnet-4" → { providerID: "anthropic", modelID: "claude-sonnet-4" }
 */
