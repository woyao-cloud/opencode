/**
 * acp/agent - Agent Client Protocol (ACP) Agent 实现
 *
 * 功能概述：
 * - 实现 ACP 协议中的 Agent 端，作为 OpenCode 与 ACP 客户端（如 IDE）之间的桥梁
 * - 处理会话生命周期（创建、加载、恢复、分叉、关闭）
 * - 管理事件订阅与分发（权限请求、消息流、工具调用更新）
 * - 支持模型选择、模式切换、命令执行和提示处理
 *
 * 核心导出：
 * - Agent：ACP Agent 主类，实现 ACP 协议的所有 Agent 端接口
 * - init：ACP Agent 初始化工厂函数
 *
 * 架构位置：OpenCode ACP 协议实现层，依赖 Session、Config、Provider、SDK 等模块，
 * 接收来自 ACP 客户端的协议请求，转发到 OpenCode 内核处理
 */
...
/**
 * 获取指定模型的上下文限制（token 上限）
 * @param sdk - OpenCode SDK 客户端
 * @param providerID - 提供商 ID
 * @param modelID - 模型 ID
 * @param directory - 工作目录
 * @returns 上下文限制（token 数），获取失败返回 null
 */
...
/**
 * 发送会话使用量更新到 ACP 客户端（token 用量、成本等）
 */
...
/**
 * ACP Agent 实现类。
 * 实现 Agent Client Protocol 中 Agent 侧的所有接口方法，
 * 负责与 ACP 客户端（如 IDE 插件）的双向通信，以及对 OpenCode 内核的调用转发。
 */
export class Agent implements ACPAgent {
  ...
  constructor(connection: AgentSideConnection, config: ACPConfig) {
    ...
    // 启动后立即订阅 OpenCode 全局事件（权限请求、消息流等）
  ...
  private async handleEvent(event: Event) {
    switch (event.type) {
      case "permission.asked": {
        // 处理 ACP 客户端的权限请求：将 OpenCode 的权限询问转发到 ACP 客户端
        const permission = event.properties
        const session = this.sessionManager.tryGet(permission.sessionID)
        if (!session) return

        // 对同一会话的权限请求进行串行化处理，避免并发冲突
        ...
        const next = prev
          .then(async () => {
            ...
            if (res.outcome.optionId !== "reject" && permission.permission == "edit") {
              // 编辑权限被允许时，计算 diff 后的新内容并发送给 ACP 客户端预览
      ...
      case "message.part.updated": {
        ...
        if (part.type === "tool") {
          // 处理工具调用状态变更：将 OpenCode 内部的工具执行进度同步到 ACP 客户端
          await this.toolStart(sessionId, part)

          switch (part.state.status) {
            case "pending":
              // 清除旧的 shell 输出快照，准备新的执行
            ...
            case "running":
              ...
              if (output) {
                ...
                if (part.tool === ShellID.ToolID) {
                  // 对 shell 工具进行去重：相同哈希表示输出未变化，跳过更新
            ...
            case "completed": {
              ...
              // 解析 todowrite 输出为计划条目，发送到 ACP 客户端
        ...
        // ACP 客户端已经知道它们刚提交的提示，重放实时用户部分会导致消息重复。
        // 但历史消息仍会在 loadSession()/forkSession() 中通过 processMessage() 重放。
  ...
  /**
   * 处理 ACP 客户端的提示请求：转换消息格式并转发到 OpenCode 会话
   */
...
/**
 * 将 OpenCode 工具名称映射为 ACP 协议的工具类型
 */
...
/**
 * 从工具输入中提取文件路径位置信息，用于 ACP 协议的 location 字段
 */
...
/**
 * 构建已完成工具调用的内容块，包括文本输出、编辑 diff 和图片附件
 */
