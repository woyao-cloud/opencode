/**
 * [effect/runtime-flags] - 运行时标志模块
 *
 * 功能概述：
 * - 定义和管理运行时特性标志（Feature Flag）
 * - 通过环境变量配置各功能的启用/禁用状态
 * - 支持实验性功能和配置覆盖
 *
 * 核心导出：
 * - Service：运行时标志服务
 * - layerWith()：创建带覆盖标志的 Layer
 * - defaultLayer：默认运行时标志 Layer
 *
 * 架构位置：上游依赖 Effect Config、config-service；下游被各功能模块引用
 */
