/**
 * [effect/instance-state] - 实例状态管理模块
 *
 * 功能概述：
 * - 提供基于 ScopedCache 的实例级状态缓存
 * - 支持按目录缓存和失效实例状态
 *
 * 核心导出：
 * - InstanceState：实例状态接口
 * - context()：创建实例状态上下文
 * - invalidate()：使实例状态失效
 *
 * 架构位置：上游依赖 Effect、instance-ref、instance-registry、workspace-context；下游被 bus/index.ts 引用
 */
