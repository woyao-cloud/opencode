/**
 * [effect/service-use] - 服务使用访问器生成模块
 *
 * 功能概述：
 * - 提供 ServiceUse 代理，将服务的 Effect 方法包装为可直接调用的访问器
 * - 自动为服务方法的 Effect 添加服务标识符依赖
 *
 * 核心导出：
 * - ServiceUse：服务使用访问器类型
 * - use()：创建服务使用代理
 *
 * 架构位置：上游依赖 Effect Context；下游被各服务使用者引用
 */
