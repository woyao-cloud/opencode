/**
 * [effect/config-service] - 配置服务基础设施
 *
 * 功能概述：
 * - 提供基于 Effect Config 的 Context 服务生成工具
 * - 自动推导服务形状（Shape）和创建带配置层的 ServiceClass
 *
 * 核心导出：
 * - Shape：从 Config 映射推导的服务形状类型
 * - Service：配置服务创建工厂函数
 *
 * 架构位置：上游依赖 Effect Config、Context；下游被 runtime-flags.ts 等引用
 */
