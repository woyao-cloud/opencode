 ...
 *
 * 功能概述：
 * - 定义通用的网络相关 yargs 选项（port、hostname、mdns、cors 等）
 * - 提供 withNetworkOptions() 为 yargs 添加网络选项
 * - resolveNetworkOptions() 基于配置解析网络选项
 *
 * 核心导出：
 * - withNetworkOptions(yargs): 向 yargs 添加网络选项
 * - resolveNetworkOptions: Effect 函数，解析网络配置
 * - NetworkOptions: 网络选项类型
 *
 * 架构位置：CLI 层工具模块，被 serve、web、acp 等命令引用
 */
