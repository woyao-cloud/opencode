/**
 * cli/effect-cmd - Effect 风格命令定义框架
 *
 * 功能概述：
 * - 基于 Effect 系统的类型安全 CLI 命令定义机制
 * - 提供 CliError / fail() 用于用户可见的命令失败处理
 * - effectCmd() 函数封装了 InstanceContext 注入、错误处理、资源释放等生命周期
 *
 * 核心导出：
 * - CliError: Schema 标签错误类，支持自定义 message 和 exitCode
 * - fail(message, exitCode?): 快速抛出 CliError 的辅助函数
 * - effectCmd(): 定义 Effect 命令的工厂函数，自动处理 InstanceContext 提供与销毁
 *
 * 架构位置：位于 cli/ 命令层核心，上游被各类具体命令调用，下游依赖 cmd/cmd.ts 的 cmd() 包装器和 effect/app-runtime 等基础设施
 */
...
export const effectCmd = <Args, A>(opts: EffectCmdOpts<Args, A>) =>
  cmd<{}, Args>({
    ...
    async handler(rawArgs) {
      ...
      // 加载 InstanceContext（配置、插件、LSP 等初始化），完成后自动释放
      const { store, ctx } = await AppRuntime.runPromise(
        InstanceStore.Service.use((store) => store.load({ directory }).pipe(Effect.map((ctx) => ({ store, ctx })))),
      )
      try {
        // 将 InstanceRef 注入到 handler 的 Effect 上下文中执行
        await AppRuntime.runPromise(opts.handler(args).pipe(Effect.provideService(InstanceRef, ctx)))
      } finally {
        // 确保 InstanceContext 在退出时释放（成功、失败、缺陷、中断均触发）
