/**
 * cli/effect/prompt - Effect 风格的 @clack/prompts 包装
 *
 * 功能概述：
 * - 将 @clack/prompts 的交互式终端提示包装为 Effect 函数
 * - 提供 intro、outro、log、select、text、spinner 等 Effect 版本
 * - isCancel 结果映射为 Option 类型
 *
 * 核心导出：
 * - intro / outro: 引导/结束提示
 * - log: 日志输出（info/error/warn/success）
 * - select / autocomplete / text / password: 交互式输入
 * - spinner: 加载动画
 *
 * 架构位置：CLI 层工具模块，被各类交互式命令 handler 引用
 */
