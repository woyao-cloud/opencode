/**
 * cli/cmd/tui/keymap - TUI 键盘映射与快捷键系统
 *
 * 功能概述：
 * - 封装 @opentui/keymap 提供 TUI 键盘快捷键绑定和匹配
 * - 定义按键别名（enter->return, esc->escape）、Leader 键等
 * - 导出 OpencodeKeymapProvider 等供应用层使用
 *
 * 核心导出：
 * - OpencodeKeymapProvider / useOpencodeKeymap: 快捷键上下文提供者与 Hook
 * - LEADER_TOKEN: 组合键引导标记常量
 *
 * 架构位置：TUI 基础设施层，被 app.tsx 和各个 UI 组件引用，依赖 config/tui 和 config/keybind
 */
