/**
 * cli/cmd/tui/config/keybind - 按键绑定类型与 Schema
 *
 * 功能概述：
 * - 定义 TUI 按键绑定的数据结构和 Schema 验证
 * - 提供 BindingCommandMap、BindingConfig 等类型
 *
 * 核心导出：
 * - TuiKeybind: 按键绑定模块的重新导出
 * - 各类 KeyStroke / Binding 类型定义
 *
 * 架构位置：TUI 配置层，被 tui-schema.ts 和 tui.ts 引用
 */
