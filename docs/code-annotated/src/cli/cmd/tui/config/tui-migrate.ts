/**
 * cli/cmd/tui/config/tui-migrate - TUI 配置迁移
 *
 * 功能概述：
 * - 处理 TUI 配置文件从旧版本到新版本的迁移
 * - 使用 jsonc-parser 进行 JSONC 格式的修改操作
 *
 * 核心导出：
 * - migrateTuiConfig(configPath): 迁移 TUI 配置文件
 *
 * 架构位置：TUI 配置层，被 tui.ts 在加载配置时调用
 */
