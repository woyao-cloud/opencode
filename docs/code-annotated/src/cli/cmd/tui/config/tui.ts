/**
 * cli/cmd/tui/config/tui - TUI 主配置模块
 *
 * 功能概述：
 * - TUI 配置加载、解析和管理
 * - 包含键位绑定查找、主题配置、注意力声音路径解析
 * - 基于 Effect Layer 提供配置上下文
 *
 * 核心导出：
 * - TuiConfig: 配置模块的重新导出
 * - TuiConfig 的 Effect Layer 和 Context Tag
 *
 * 架构位置：TUI 配置层核心，被 app.tsx、layer.ts、attention.ts 等引用
 */
