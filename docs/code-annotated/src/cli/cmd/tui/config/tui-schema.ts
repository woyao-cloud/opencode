/**
 * cli/cmd/tui/config/tui-schema - TUI 配置 Schema 定义
 *
 * 功能概述：
 * - 定义 TUI 配置的 Effect Schema 数据结构
 * - 包含注意力通知声音路径解析、主题等配置项
 *
 * 核心导出：
 * - TuiInfo: TUI 配置信息的 Schema
 * - resolveAttentionSoundPaths / isAttentionSoundName 等工具函数
 *
 * 架构位置：TUI 配置层，被 tui.ts 和 attention.ts 引用
 */
