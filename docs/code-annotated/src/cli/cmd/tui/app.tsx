/**
 * cli/cmd/tui/app - TUI 主应用入口与路由
 *
 * 功能概述：
 * - 基于 @opentui/solid 的终端 UI 应用根组件
 * - 管理全局路由（session/home 等）、主题、同步、对话框等上下文提供者
 * - 处理 Ctrl+C 等平台适配（win32）和生命周期
 *
 * 核心导出：
 * - App 组件（默认导出）：TUI 应用根组件
 * - AppRouter: 路由表常量
 *
 * 架构位置：TUI 顶层入口，组合所有 context provider 和路由匹配，下游引用 context/*、component/*、route/* 等各子模块
 */
