/**
 * cli/cmd/tui/context/aggregate-failures - 聚合错误处理工具
 *
 * 功能概述：
 * - 将 Promise.allSettled 结果聚合成单个 Error，列出所有失败的端点
 * - 用于 TUI 启动时多个并行请求的错误汇总
 *
 * 核心导出：
 * - aggregateFailures(results): 聚合失败结果为单一错误或 null
 *
 * 架构位置：TUI 上下文层，被 TUI 启动引导流程使用
 */
