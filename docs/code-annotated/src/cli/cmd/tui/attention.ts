/**
 * cli/cmd/tui/attention - TUI 注意力通知系统
 *
 * 功能概述：
 * - 管理终端注意力通知（声音/视觉提醒）的注册、触发和生命周期
 * - 支持焦点状态跟踪、声音包注册和内置音频资源
 *
 * 核心导出：
 * - 各种 TuiAttention 相关类型和内部渲染器管理
 *
 * 架构位置：TUI 系统功能层，被 TUI 路由组件引用，依赖 config/tui 和 util/audio 等模块
 */
