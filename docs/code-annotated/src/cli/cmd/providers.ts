/**
 * cli/cmd/providers - "opencode providers" 命令
 *
 * 功能概述：
 * - 实现 providers 命令，管理 AI 提供商配置
 * - 支持查看、添加、修改和删除提供商设置
 *
 * 核心导出：
 * - ProvidersCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 auth 模块和 provider 模块
 */
