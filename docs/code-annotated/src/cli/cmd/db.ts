/**
 * cli/cmd/db - "opencode db" 命令
 *
 * 功能概述：
 * - 实现 db 命令，提供交互式 SQLite shell 或直接执行查询
 * - 用于调试和直接操作 opencode 数据库
 *
 * 核心导出：
 * - QueryCommand: 使用 cmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层（调试工具），被 cli/cmd/ 索引注册，依赖 storage/db 和 drizzle ORM
 */
