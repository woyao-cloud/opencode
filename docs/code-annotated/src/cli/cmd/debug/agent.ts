/**
 * cli/cmd/debug/agent - "opencode debug agent" 命令
 *
 * 功能概述：
 * - 调试 agent 相关功能，包括查看 agent 配置和会话消息
 *
 * 核心导出：
 * - AgentCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI debug 命令组，被 debug/index.ts 注册，依赖 agent、provider、session 模块
 */
