/**
 * cli/cmd/models - "opencode models" 命令
 *
 * 功能概述：
 * - 实现 models 命令，列出可用的 AI 模型
 * - 支持按 provider 筛选模型列表
 *
 * 核心导出：
 * - ModelsCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 provider 模块和 models 模块
 */
