/**
 * RunCommand - opencode run 命令的完整实现
 *
 * 功能概述：
 * - 非交互模式（默认）：发送单次 prompt，将事件流输出到 stdout，会话空闲时退出
 * - 交互式本地模式（--interactive）：启动进程内服务，使用分割式 Footer 直接模式
 * - 交互式附加模式（--interactive --attach）：连接到正在运行的 opencode 服务
 * - 支持 --command 执行斜杠命令、--format json 输出原始事件流
 * - 支持 --continue / --session 恢复会话、--fork 在继续前派生新会话
 *
 * 核心导出：
 * - RunCommand: 使用 effectCmd 封装的 yargs 命令定义，包含 builder 和 handler
 *
 * 架构位置：opencode CLI 核心入口，位于 src/cli/cmd/ 层，下游依赖 runtime 模块、SDK 客户端和服务端
 */
...
/** 将 "provider/model" 格式的字符串解析为 SDK 的 ModelInput 对象 */
...
/** 在 stdout 上输出一行内联信息（图标 + 标题 + 可选描述） */
function inline(info: Inline) {
  const suffix = info.description ? UI.Style.TEXT_DIM + ` ${info.description}` + UI.Style.TEXT_NORMAL : ""
  UI.println(UI.Style.TEXT_NORMAL + info.icon, UI.Style.TEXT_NORMAL + info.title + suffix)
}

/** 在 stdout 上输出块级信息（图标 + 标题 + 可选描述 + 正文内容） */
...
/** 格式化运行时的错误信息，优先使用 FormatError，回退到 FormatUnknownError */
...
export const RunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      const rawMessage = [...args.message, ...(args["--"] || [])].join(" ") // 合并 positional 参数和 -- 后的参数
      const thinking = args.interactive ? (args.thinking ?? true) : (args.thinking ?? false) // 交互模式下默认显示 thinking
      ...
      const rules: Permission.Ruleset = args.interactive
        ? [] // 交互模式下不限制权限，由用户在 UI 中决定
        : [
            {
              permission: "question",
              action: "deny",
              pattern: "*", // 非交互模式自动拒绝所有问题
            },
            {
              permission: "plan_enter",
              action: "deny",
              pattern: "*", // 非交互模式自动拒绝所有 plan 进入
            },
            {
              permission: "plan_exit",
              action: "deny",
              pattern: "*", // 非交互模式自动拒绝所有 plan 退出
            },
          ]

      function title() {
        if (args.title === undefined) return // 未提供 title，由服务端自动生成
        if (args.title !== "") return args.title // 用户指定了明确的 title
        return message.slice(0, 50) + (message.length > 50 ? "..." : "") // title 为空字符串时截取消息前 50 字符
      }

      /**
       * 获取或创建会话。支持按 sessionID 恢复、fork 派生新会话、continue 继续最近会话。
       * @returns 会话信息（id, title, directory）或 undefined
       */
      async function session(sdk: OpencodeClient): Promise<SessionInfo | undefined> {
        // 尝试按 sessionID 获取现有会话，如果未找到则报错退出
        if (args.session) {
