/**
 * cli/cmd/stats - "opencode stats" 命令
 *
 * 功能概述：
 * - 实现 stats 命令，统计和展示会话使用数据
 * - 统计总会话数、消息数、Token 用量和花费
 *
 * 核心导出：
 * - StatsCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 session 模块和 storage/db 模块
 */
