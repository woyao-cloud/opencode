/**
 * cli/cmd/run/splash.ts - 进入/退出欢迎横幅
 *
 * 功能概述：
 * - 渲染完整的 opencode 进入标志和紧凑的 [O] 退出标识
 * - 显示会话元数据和恢复命令
 * - 生成不可变的 scrollback 快照，提交后成为终端历史
 *
 * 核心导出：
 * - entrySplash: 进入欢迎横幅
 * - exitSplash: 退出标识
 *
 * 架构位置：CLI run 命令子模块的展示层，
 * 被 runtime.lifecycle 在启动和关闭时写入 scrollback
 */
