/**
 * cli/cmd/run/otel.ts - 遥测（OpenTelemetry）集成
 *
 * 功能概述：
 * - 集成 OpenTelemetry 追踪，为交互模式提供 Span 创建和管理
 * - 通过 Effect 系统的 ManagedRuntime 与 memoMap 管理追踪实例
 *
 * 核心导出：
 * - 创建和管理追踪 Span 的 Effect 服务
 *
 * 架构位置：CLI run 命令子模块的遥测层，
 * 被 runtime 和其他模块用于性能追踪和诊断
 */
