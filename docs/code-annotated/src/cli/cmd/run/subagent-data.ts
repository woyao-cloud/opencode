/**
 * cli/cmd/run/subagent-data.ts - 子代理（Subagent）数据状态管理
 *
 * 功能概述：
 * - 管理子代理会话的数据结构（SubagentData），包含标签页和详细帧
 * - 提供创建、引导、缩减、快照等纯函数操作
 * - 同步 task 工具的 part 状态到子代理标签页
 * - 处理子代理的事件分发、commit 去重与紧凑存储
 *
 * 核心导出：
 * - SubagentData: 子代理数据结构（tabs / details）
 * - createSubagentData / bootstrapSubagentData / bootstrapSubagentCalls
 * - reduceSubagentData: 事件驱动的子代理状态缩减
 * - snapshotSubagentData / snapshotSelectedSubagentData: 快照导出
 * - listSubagentPermissions / listSubagentQuestions / listSubagentTabs
 * - clearFinishedSubagents: 清除已完成的子代理
 * - sameSubagentTab: 标签页状态比较
 * - SUBAGENT_BOOTSTRAP_LIMIT / SUBAGENT_CALL_BOOTSTRAP_LIMIT: 限制常量
 *
 * 架构位置：CLI run 命令子模块，被 stream.transport 调用的 reducer，
 * 与 session-data 配合完成会话树的多级事件处理
 */
