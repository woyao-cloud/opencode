/**
 * cli/cmd/run/stream.ts - reducer 输出到 footer API 的桥梁
 *
 * 功能概述：
 * - 将 reducer 产出的 StreamCommit[] 和 FooterOutput 转发到 footer
 * - 提供 writeSessionOutput() 和 traceFooterOutput() 两个转发函数
 * - 在转发过程中添加 trace 日志写入
 *
 * 核心导出：
 * - writeSessionOutput: 将 commits 和 footer output 写入 footer API
 * - traceFooterOutput: 从 FooterOutput 提取可追踪的摘要信息
 *
 * 架构位置：CLI run 命令子模块，位于 reducer 和 footer API 之间的薄桥接层，
 * 被 stream.transport 调用
 */
