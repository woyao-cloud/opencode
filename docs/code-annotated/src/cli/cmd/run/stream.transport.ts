/**
 * cli/cmd/run/stream.transport.ts - 全局事件订阅与 Prompt Turn 协调
 *
 * 功能概述：
 * - 创建长期全局事件流订阅，将当前会话树的 SDK 事件馈入 reducer
 * - reducer 产出 scrollback commits 和 footer patches，通过 stream.ts 转发
 * - runPromptTurn() 单次发送 prompt 到 SDK，通过 Wait 延迟等待会话 idle
 * - 使用 tick 计数器防止陈旧 idle 事件错误解析新 turn
 *
 * 核心导出：
 * - createSessionTransport: 创建 SDK 事件订阅并返回 SessionTransport
 * - SessionTransport: 包含 runPromptTurn / selectSubagent / close 接口
 * - formatUnknownError: 格式化未知错误为字符串
 *
 * 架构位置：CLI run 命令子模块，stream.transport 是 reducer 与 SDK 之间的桥梁，
 * 接收 SDK 事件并驱动 session-data 和 subagent-data 的 reducer
 */
