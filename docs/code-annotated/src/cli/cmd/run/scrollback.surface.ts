/**
 * cli/cmd/run/scrollback.surface.ts - 流式 Scrollback 保留表面
 *
 * 功能概述：
 * - 保留直接模式 scrollback 的流式追加逻辑
 * - 静态条目通过 scrollback.writer.tsx 渲染，本文件仅处理流式助手、
 *   推理和工具进度条目所需的保留表面机制
 * - 确保流式输出保持稳定的 markdown/代码布局
 *
 * 核心导出：
 * - createScrollbackSurface: 创建保留表面
 * - 流式条目渲染的核心机制
 *
 * 架构位置：CLI run 命令子模块的 scrollback 层，
 * 与 scrollback.writer.tsx 配合完成滚动区域的渲染
 */
