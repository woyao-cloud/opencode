/**
 * cli/cmd/run/session-data.ts - 核心会话数据 Reducer
 *
 * 功能概述：
 * - 接收原始 SDK 事件并产出两种输出：StreamCommit[]（追加式 scrollback 条目）
 *   和 FooterOutput（状态栏补丁和视图切换）
 * - 管理会话状态数据结构（SessionData），包含文本、工具、权限、问答等
 * - 提供 createSessionData / bootstrapSessionData / reduceSessionData 等函数
 *
 * 核心导出：
 * - SessionData: 会话状态数据结构
 * - createSessionData / bootstrapSessionData / reduceSessionData
 * - flushInterrupted / pickBlockerView / blockerStatus
 * - formatError: 错误格式化
 *
 * 架构位置：CLI run 命令子模块的核心 reducer，
 * 被 stream.transport 和 subagent-data 调用
 */
