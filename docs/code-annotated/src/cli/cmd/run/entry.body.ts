/**
 * cli/cmd/run/entry.body.ts - Scrollback 条目正文生成
 *
 * 功能概述：
 * - 根据 StreamCommit 生成 scrollback 条目的正文内容和展示标志
 * - 提供纯函数用于文本清理、完成判断、flags 计算
 *
 * 核心导出：
 * - entryBody: 从 StreamCommit 生成条目正文
 * - entryFlags: 计算条目的展示标志（换行、缩进等）
 * - entryDone: 判断条目是否完成
 * - cleanRunText: 清理运行文本
 * - RUN_ENTRY_NONE: 空条目常量
 * - EntryFlags / RunEntryBody: 类型定义
 *
 * 架构位置：CLI run 命令子模块的 scrollback 层，
 * 被 scrollback.writer.tsx 用于渲染每个条目
 */
