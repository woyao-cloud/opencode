/**
 * cli/cmd/run/session.shared.ts - 会话消息提取与提示历史
 *
 * 功能概述：
 * - 从 SDK 获取会话消息并提取用户 turn 文本供提示历史环使用
 * - 查找当前模型的最近使用变体，供 footer 预选
 *
 * 核心导出：
 * - fetchSessionMessages: 获取会话消息
 * - extractUserTurns: 提取用户 turn 文本
 * - findRecentVariant: 查找最近使用的变体
 *
 * 架构位置：CLI run 命令子模块的会话层，
 * 被 runtime.boot 用于启动时初始化提示历史
 */
