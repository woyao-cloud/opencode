/**
 * cli/cmd/run/runtime.boot.ts - 交互模式启动时依赖解析
 *
 * 功能概述：
 * - 启动时并发收集运行时所需的所有配置：键绑定、差异显示样式、
 *   模型变体列表及上下文限制、会话历史
 * - 在首帧渲染前完成所有异步初始化解锁
 *
 * 核心导出：
 * - bootstrapRun: 并发引导函数，返回所有启动参数
 * - BootstrapRunInput / BootstrapRunOutput: 输入输出类型
 *
 * 架构位置：CLI run 命令子模块的启动阶段，
 * 被 runtime.lifecycle 在创建渲染器之前调用
 */
