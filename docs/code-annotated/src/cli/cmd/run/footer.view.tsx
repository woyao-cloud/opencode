/**
 * RunFooterView - 终端 Footer 状态栏
 *
 * 功能概述：
 * - 渲染交互模式下的底部状态栏区域，作为垂直堆叠布局：
 *   1. 间隔行（与滚动区域的可视分隔）
 *   2. 编辑器框架（带左侧边框装饰），通过 Switch/Match 在 prompt、permission、question 间切换
 *   3. 元信息行（agent 名称和模型标签）
 *   4. 底部边框 + 状态行（spinner、中断提示、持续时间、用量）
 * - 所有状态通过 SolidJS signals 从父级 RunFooter 传入，视图本身无状态（仅派生 memos）
 * - 支持子代理（subagent）标签页、命令菜单、模型选择、变体选择等面板
 *
 * 核心导出：
 * - RunFooterView：Footer 组件
 * - TEXTAREA_MIN_ROWS / TEXTAREA_MAX_ROWS：从 footer.prompt 转发
 *
 * 架构位置：run 交互模式的 UI 渲染层，位于 footer 区域顶层，内部组合 footer.prompt、
 * footer.permission、footer.question、footer.command、footer.menu、footer.subagent 等子组件
 */
...
/**
 * RunFooterView 组件 - 终端底部状态栏
 *
 * 根据当前视图类型（prompt / permission / question / command / model / variant）
 * 和子代理标签页状态，渲染对应的 footer 区域布局。
 * 所有状态通过 SolidJS 信号驱动，包含键盘快捷键处理（命令面板、子代理切换）。
 */
