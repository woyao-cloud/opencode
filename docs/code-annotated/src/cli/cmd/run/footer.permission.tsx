/**
 * cli/cmd/run/footer.permission.tsx - Permission 权限 UI 组件
 *
 * 功能概述：
 * - 渲染 footer 中的权限请求 UI，当 reducer 推送 "permission" 视图时展示
 * - 使用 permission.shared.ts 的三阶段状态机：permission / always / reject
 * - 键盘操作：左右键选择、回车确认、Esc 拒绝
 * - 支持差异对比视图（与 scrollback tool snapshot 共用 diff 组件）
 *
 * 核心导出：
 * - RunPermissionBody: 权限请求 UI 组件
 *
 * 架构位置：CLI run 命令子模块 footer 区域，被 footer.view 调用，
 * 依赖 permission.shared 状态机
 */
