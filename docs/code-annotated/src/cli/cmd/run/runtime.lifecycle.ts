/**
 * cli/cmd/run/runtime.lifecycle.ts - split-footer 渲染器生命周期管理
 *
 * 功能概述：
 * - 创建 OpenTUI CliRenderer 的 split-footer 模式
 * - 从终端调色板解析主题，写入进入 splash 到 scrollback
 * - 构造 RunFooter，返回 Lifecycle 句柄
 * - close() 按正确顺序关闭：footer.close -> footer.destroy -> renderer shutdown
 * - 绑定 SIGINT：首次 Ctrl-c 清除当前 prompt draft，再次 Ctrl-c 触发退出
 *
 * 核心导出：
 * - createRunLifecycle: 创建运行生命周期
 * - RunLifecycle: 生命周期句柄（close 方法）
 *
 * 架构位置：CLI run 命令子模块的启动和关闭编排层，
 * 在 run 命令入口处创建，协调所有子模块的初始化与销毁
 */
