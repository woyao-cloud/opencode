/**
 * cli/cmd/run/prompt.shared.ts - Prompt 输入纯状态机
 *
 * 功能概述：
 * - 处理键位解析、历史环导航和变体循环的 leader 键序列
 * - 所有函数为纯函数（无副作用）：输入状态，输出新状态
 * - 历史环（PromptHistoryState）存储过去的 prompt 并跟踪当前浏览位置
 * - leader 键循环（promptCycle）使用两步模式：首次按下武装 leader，
 *   超时内第二次按下触发操作
 *
 * 核心导出：
 * - PromptState / PromptHistoryState: 状态类型
 * - promptReducer: prompt 状态缩减器
 * - promptCycle: leader 键循环处理
 *
 * 架构位置：CLI run 命令子模块的输入层，
 * 被 footer.prompt.tsx 用于驱动 prompt 输入组件
 */
