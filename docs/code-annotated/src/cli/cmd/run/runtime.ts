/**
 * Runtime - 运行时生命周期管理（交互模式）
 *
 * 功能概述：
 * - runInteractiveMode：附加模式入口，使用调用方提供的 SDK 客户端
 * - runInteractiveLocalMode：本地进程内模式入口，创建内联 fetch 的 SDK 客户端
 * - runInteractiveRuntime：核心运行时循环，协调启动、生命周期、流传输和 prompt 队列
 * - 处理 session 恢复、模型/变体切换、subagent 选择、中断等运行时状态
 *
 * 核心导出：
 * - runInteractiveMode：附加模式入口
 * - runInteractiveLocalMode：本地模式入口
 * - runPromptQueue：prompt 队列（导出用于测试）
 * - pickVariant / resolveVariant：变体选择（导出用于测试）
 *
 * 架构位置：位于 run 命令的交互模式核心层，上游被 run.ts 调用，下游依赖 runtime.boot、
 * runtime.lifecycle、stream.transport、runtime.queue 等子模块
 */
...
/** 将可选的外部 createSession 函数包装为内部 ResolvedSession 格式 */
  ...
  return withRunSpan(
    ...
    async (span) => {
      ...
      const shell = await createRuntimeLifecycle({
        ...
        findFiles: (query) => // 文件查找委托给 SDK 的 find.files 接口
          ctx.sdk.find
        ...
        first: session.first, // 是否首次会话（决定是否显示初始 splash）
        history: session.history,
        agent: state.agent,
        model: state.model,
        ...
        keybinds,
        diffStyle,
        onPermissionReply: async (next) => { // 权限回复回调，demo 模式可拦截
        ...
        onInterrupt: () => { // 中断当前会话，设置 aborting 标志防止重复触发
