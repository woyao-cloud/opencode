 ...
 *
 * 功能概述：
 * - 定义每个已知工具（bash、edit、write、task 等）的显示规则 TOOL_RULES
 * - 提供五种显示钩子：view（可见性策略）、run（内联摘要）、scroll（滚动文本）、
 *   permission（权限 UI）、snap（结构化快照）
 * - 未注册的工具使用 fallback 格式
 * - 包含辅助函数：toolInlineInfo、toolScroll、toolPermissionInfo、toolSnapshot 等
 *
 * 核心导出：
 * - TOOL_RULES：工具显示规则注册表
 * - toolView：获取工具的可视策略
 * - toolInlineInfo：获取非交互模式下的内联展示信息
 * - toolScroll：获取滚动区域中某阶段的文本
 * - toolPermissionInfo：获取权限请求的展示信息
 * - toolSnapshot：获取结构化快照
 * - toolEntryBody：根据 commit 和 phase 生成条目正文
 * - toolFrame：从 StreamCommit 构建 ToolFrame
 *
 * 架构位置：run 命令的输出渲染层，被 run.ts 和 stream.transport 调用
 */
...
/**
 * 将文件路径转换为相对路径（相对于 cwd），可选支持 home 目录缩写
 * @param input 原始路径
 * @param opts.home 是否使用 ~ 表示 home 目录
 */
...
/** 从 StreamCommit 和原始内容构建 ToolFrame 对象 */
...
/** 根据 ToolPart 获取内联展示信息（用于非交互模式 stdout 输出） */
...
/** 获取滚动区域中某阶段（start/progress/final）的格式文本，无规则时使用 fallback */
...
/** 获取权限请求的 UI 展示信息（图标、标题、描述行、diff），无规则时返回 undefined */
...
/** 根据 commit 和原始内容获取结构化快照（code、diff、task 等），无规则时返回 undefined */
...
/** 根据 commit 和 phase 生成滚动区域的条目正文（text/markdown/structured），按可见策略过滤 */
...
/** 根据文件扩展名返回语法高亮的语言类型，将 tsx/jsx/js 统一归为 typescript */
