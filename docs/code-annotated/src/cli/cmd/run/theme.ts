/**
 * cli/cmd/run/theme.ts - 交互模式主题解析
 *
 * 功能概述：
 * - 从终端实际调色板派生 scrollback 和 footer 颜色
 * - resolveRunTheme() 查询渲染器获取终端调色板，检测明暗模式
 * - 构建小型系统主题并映射到 scrollback 和 footer 主题
 *
 * 核心导出：
 * - RunTheme / RunScrollbackTheme / RunFooterTheme: 主题类型
 * - resolveRunTheme: 解析终端调色板并构建主题
 * - transparent: 透明颜色常量
 *
 * 架构位置：CLI run 命令子模块的主题层，
 * 被 runtime.lifecycle 和所有 UI 组件用于颜色解析
 */
