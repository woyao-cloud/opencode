/**
 * cli/cmd/run/footer.question.tsx - Question 问答 UI 组件
 *
 * 功能概述：
 * - 渲染 footer 中的问答请求 UI，当 reducer 推送 "question" 视图时展示
 * - 支持单问题（选择后立即提交）和多问题（标签页 + 最终确认）两种流程
 * - 键盘操作：左右键选择、回车确认、Esc 返回
 *
 * 核心导出：
 * - RunQuestionBody: 问答请求 UI 组件
 *
 * 架构位置：CLI run 命令子模块 footer 区域，被 footer.view 调用，
 * 依赖 question.shared 状态机
 */
