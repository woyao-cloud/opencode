/**
 * cli/cmd/run/types.ts - 交互模式共享类型定义
 *
 * 功能概述：
 * - 定义 split-footer 终端布局所需的所有共享类型
 * - 包含 scrollback 条目（StreamCommit）、footer 视图（FooterView）、
 *   状态补丁（FooterPatch）、输出（FooterOutput）等
 * - 定义 prompt 输入（RunPrompt / RunInput / RunFilePart / RunPromptPart）
 *   和 footer API（FooterApi / FooterEvent）接口
 * - 定义子代理状态（FooterSubagentState / FooterSubagentTab）类型
 *
 * 核心导出：
 * - StreamCommit: scrollback 条目
 * - FooterView / FooterPatch / FooterOutput: footer 视图相关
 * - RunPrompt / RunInput / RunFilePart / RunPromptPart: prompt 输入
 * - FooterApi / FooterEvent: footer API 接口
 * - FooterSubagentState / FooterSubagentTab: 子代理状态
 *
 * 架构位置：CLI run 命令子模块的类型基础，被所有 run/* 模块引用
 */
