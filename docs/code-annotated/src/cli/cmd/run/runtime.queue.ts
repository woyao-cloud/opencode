/**
 * cli/cmd/run/runtime.queue.ts - 串行 Prompt 队列
 *
 * 功能概述：
 * - 来自 footer 的 prompt（用户输入后回车）在此排队
 * - 队列一次处理一个 turn：追加用户行到 scrollback，
 *   调用 input.run() 执行 turn，等待完成后再开始下一个 prompt
 * - 处理 /exit、/quit、/new 命令和空 prompt 拒绝
 * - 跟踪每次 turn 的实时耗时供 footer 状态行使用
 * - 在 footer 关闭且所有进行中的工作完成时解析
 *
 * 核心导出：
 * - runPromptQueue: 运行 prompt 队列直到 footer 关闭
 *
 * 架构位置：CLI run 命令子模块的执行协调层，
 * 位于 footer 和 stream.transport 之间，管理 turn 的串行执行
 */
