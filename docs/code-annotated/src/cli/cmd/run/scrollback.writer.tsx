/**
 * cli/cmd/run/scrollback.writer.tsx - Scrollback 条目渲染组件
 *
 * 功能概述：
 * - 使用 OpenTUI ScrollbackWriter 渲染 scrollback 中的每个条目
 * - 根据 StreamCommit 的类型（文本、工具、错误、推理等）切换渲染策略
 * - 组合 entry.body 的内容生成和 scrollback.shared 的样式
 *
 * 核心导出：
 * - ScrollbackEntry: scrollback 条目渲染组件
 *
 * 架构位置：CLI run 命令子模块的 UI 渲染层，
 * 被 footer.ts 用于将 commits 渲染到 scrollback 区域
 */
