/**
 * cli/cmd/run/demo.ts - 交互模式 Demo 测试
 *
 * 功能概述：
 * - 无需真实 SDK 即可测试直接交互模式的 Demo 模式
 * - 通过 `--demo` 启用，拦截 prompt 提交并生成合成 SDK 事件
 * - 支持 /permission、/question、/fmt 等斜杠命令触发不同预览场景
 * - 在本地处理权限和问答回复，完成或失败合成工具部分
 *
 * 核心导出：
 * - createRunDemo: 创建 Demo 运行环境
 *
 * 架构位置：CLI run 命令子模块的测试工具，
 * 在 --demo 模式下替代真实 SDK 连接
 */
