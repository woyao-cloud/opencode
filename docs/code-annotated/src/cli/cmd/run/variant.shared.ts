/**
 * cli/cmd/run/variant.shared.ts - 模型变体解析与持久化
 *
 * 功能概述：
 * - 变体是各供应商的推理努力等级（如 "high"、"max"）
 * - 解析优先级：CLI --variant 标志 > 已保存偏好 > 会话历史
 * - 已保存的变体持久化在 ~/.local/state/opencode/model.json
 * - 循环切换（ctrl+t）同时更新活动变体和持久化文件
 *
 * 核心导出：
 * - resolveVariant: 解析活动变体
 * - fitVariant: 检查并适配可用变体列表
 * - saveVariant: 持久化变体偏好
 * - loadSavedVariant: 加载已保存的变体
 *
 * 架构位置：CLI run 命令子模块的配置层，
 * 被 runtime.boot 和 footer 用于模型变体管理
 */
