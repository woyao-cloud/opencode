/**
 * cli/cmd/run/footer.ts - RunFooter 交互模式控制面
 *
 * 功能概述：
 * - 在 split-footer 架构中管理可变 footer 区域（scrollback 不可变）
 * - 提供 append() 追加 scrollback entries，event() 发送 footer 视图/状态更新
 * - 管理 prompt 输入事件订阅和 exit 请求状态机
 * - 支持子代理标签页选择和菜单/命令面板切换
 *
 * 核心导出：
 * - RunFooter: 通过 createRunFooter() 创建的可变 footer 控制面
 * - FooterApi: footer 的公共接口类型（append / event / close 等）
 *
 * 架构位置：CLI run 命令子模块，是 scrollback 和 footer 视图之间的控制枢纽，
 * 被 runtime.lifecycle 和 runtime.queue 调用
 */
