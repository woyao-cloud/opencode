/**
 * cli/cmd/run/permission.shared.ts - 权限 UI 纯状态机
 *
 * 功能概述：
 * - 独立于 JSX 组件的纯函数状态机，可独立测试
 * - 三阶段状态机：permission（显示请求及按钮）、always（永久授权确认）、
 *   reject（输入拒绝理由）
 *
 * 核心导出：
 * - createPermissionState: 创建权限状态
 * - PermissionState: 状态类型
 *
 * 架构位置：CLI run 命令子模块的共享逻辑层，
 * 被 footer.permission.tsx 用于驱动权限 UI
 */
