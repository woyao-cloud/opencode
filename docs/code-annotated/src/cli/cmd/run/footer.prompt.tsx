/**
 * Prompt - Footer Prompt 组件与状态机
 *
 * 功能概述：
 * - createPromptState() 创建 prompt 状态机，管理文本输入、历史导航、leader 键序列
 * - 支持 @ 自动补全：文件、子代理（subagent）、MCP 资源
 * - 支持 / 斜杠命令自动补全（内建 /new、/exit + 自定义命令）
 * - 使用 fuzzysort 实现模糊搜索补全
 * - RunPromptBody 组件渲染 OpenTUI textarea
 * - 管理 extmarks 用于高亮 @mention 引用
 *
 * 核心导出：
 * - createPromptState: 创建 Prompt 状态机
 * - RunPromptBody: Prompt 输入框 UI 组件
 * - hintFlags: 根据终端宽度判断提示文字可见性
 * - TEXTAREA_MIN_ROWS / TEXTAREA_MAX_ROWS / PROMPT_MAX_ROWS: 行数常量
 *
 * 架构位置：run 交互模式的输入层，被 footer.view 和 runtime.lifecycle 调用
 */
 ...
 * RunPromptBody - Prompt 输入框 UI 组件
 *
 * 渲染 OpenTUI textarea，绑定键盘快捷键、placeholder 和事件处理器。
 * 通过 ref 回调将 textarea 实例传递给父级状态机。
 */
 ...
 * 创建 Prompt 状态机
 *
 * 管理 textarea 的完整状态：文本编辑、@mention 自动补全（文件/子代理/MCP 资源）、
 * / 斜杠命令补全、历史导航、leader 键序列、extmarks 标记管理等。
 *
 * @param input - PromptInput 配置
 * @returns PromptState - 包含 placeholder、bindings、visible、options 等状态和操作方法
 */
