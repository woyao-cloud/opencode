/**
 * cli/cmd/mcp - "opencode mcp" 命令
 *
 * 功能概述：
 * - 实现 mcp 命令，管理 MCP（Model Context Protocol）服务器连接
 * - 支持添加、列出、配置 MCP 服务器及 OAuth 认证流程
 *
 * 核心导出：
 * - McpCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 MCP 模块和 config/mcp 模块
 */
