/**
 * cli/cmd/import - "opencode import" 命令
 *
 * 功能概述：
 * - 实现 import 命令，从 SDK v2 格式导入会话数据
 * - 支持从文件导入会话、消息和部件数据
 *
 * 核心导出：
 * - ImportCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 session 模块和 share-next 模块
 */
