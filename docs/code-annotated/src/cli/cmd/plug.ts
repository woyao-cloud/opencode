/**
 * cli/cmd/plug - "opencode plug" 命令
 *
 * 功能概述：
 * - 实现 plug 命令，管理 opencode 插件安装与配置
 * - 支持从目标源安装插件、更新插件配置和查看插件清单
 *
 * 核心导出：
 * - PlugCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 plugin 安装模块和 config 模块
 */
