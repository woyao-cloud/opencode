/**
 * cli/cmd/account - "opencode account" 命令
 *
 * 功能概述：
 * - 实现 account 命令，管理 opencode 账户登录/登出
 * - 支持浏览器 OAuth 登录流程和账户信息查看
 *
 * 核心导出：
 * - AccountCommand: 使用 effectCmd 封装的 yargs 命令定义
 *
 * 架构位置：CLI 命令层，被 cli/cmd/ 索引注册，依赖 account 模块和 effect/prompt
 */
