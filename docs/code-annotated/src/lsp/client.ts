/**
 * lsp/client - LSP 客户端：管理 LSP 连接生命周期、诊断收集与推送/拉取机制
 *
 * 功能概述：
 * - 建立与 LSP 服务器的 JSON-RPC 连接
 * - 管理文件打开/变更通知和诊断结果收集
 * - 支持推送诊断（textDocument/publishDiagnostics）和拉取诊断（textDocument/diagnostic）
 * - 诊断去重、合并和缓存
 *
 * 核心导出：
 * - create：创建 LSP 客户端实例
 * - Event.Diagnostics：诊断更新总线事件
 * - InitializeError：初始化错误类型
 *
 * 架构位置：LSP 模块底层，被 lsp/lsp.ts 服务层调用。
 * 上游依赖：lsp/server.ts（服务器句柄）、effect/instance-ref（实例引用）
 */
