/**
 * lsp/server - LSP 服务器定义：各语言 LSP 服务器的配置和启动逻辑
 *
 * 功能概述：
 * - 定义 30+ 种 LSP 服务器的配置（TypeScript、Rust、Python、Go、Java 等）
 * - 每台服务器包含 ID、文件扩展名匹配、项目根目录发现和进程启动逻辑
 * - 支持自动下载和安装缺失的 LSP 服务器
 *
 * 核心导出：
 * - Info：LSP 服务器信息接口
 * - Deno、Typescript、RustAnalyzer、Clangd 等各服务器实现
 *
 * 架构位置：LSP 模块数据/启动层，被 lsp/lsp.ts 服务层调用。
 * 上游依赖：lsp/launch（进程启动）、util/process、util/which
 */
