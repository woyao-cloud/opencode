/**
 * lsp/lsp - LSP 服务层：核心 LSP 功能集成
 *
 * 功能概述：
 * - LSP 服务器发现、启动和生命周期管理
 * - 提供诊断、悬停、定义跳转、引用查找、符号搜索等 LSP 功能
 * - 支持调用层次结构（incoming/outgoing calls）
 * - 运行时标志控制实验性服务器（如 Ty）
 *
 * 核心导出：
 * - Service：LSP 服务 Context 类
 * - layer：LSP 服务 Effect Layer
 * - Event.Updated：LSP 状态更新总线事件
 *
 * 架构位置：LSP 模块服务层，是 LSP 功能的核心入口。
 * 上游依赖：lsp/client（客户端）、lsp/server（服务器定义）、config/config（配置）
 */
