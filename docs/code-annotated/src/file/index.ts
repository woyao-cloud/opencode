/**
 * file/index - 文件服务层：管理文件的搜索、差异对比和内容操作
 *
 * 功能概述：
 * - 提供文件搜索（模糊搜索、Glob 搜索）
 * - 生成文件差异补丁
 ...
 *
 * 核心导出：
 * - Service：文件服务 Context 类
 * - layer：文件服务 Effect Layer
 *
 * 架构位置：File 模块服务层，提供统一的文件操作接口。
 * 上游依赖：file/ignore（忽略规则）、file/protected（保护路径）、file/ripgrep（搜索）
 */
