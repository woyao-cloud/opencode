/**
 * file/watcher - 文件监听器：基于 @parcel/watcher 实现文件系统变更监听
 *
 * 功能概述：
 * - 使用 @parcel/watcher 监听文件系统变更
 * - 过滤 Git 忽略文件
 * - 发布文件变更总线事件
 * - 批量合并变更事件
 *
 * 核心导出：
 * - Service：文件监听服务 Context 类
 * - layer：文件监听 Effect Layer
 *
 * 架构位置：File 模块监听层，提供实时的文件系统变更通知。
 * 上游依赖：file/ignore（忽略规则）、git（Git 忽略检查）
 */
