/**
 * reference/reference - 代码参考服务：管理外部代码库的引用和同步
 *
 * 功能概述：
 * - 管理外部代码仓库的引用配置
 * - 克隆和同步引用的仓库
 ...
 *
 * 核心导出：
 * - Service：引用服务 Context 类
 * - layer：引用服务 Effect Layer
 * - Resolved：解析后的引用信息
 *
 * 架构位置：Reference 模块服务层，提供外部代码引用能力。
 * 上游依赖：reference/repository-cache（仓库缓存）、git（Git 服务）
 */
