/**
 * mcp/auth - MCP OAuth 认证：管理 MCP 服务器的 OAuth 令牌存储和客户端信息
 *
 * 功能概述：
 * - 定义 MCP OAuth 令牌（access/refresh token）Schema
 * - 定义 OAuth 客户端信息 Schema
 * - 提供令牌持久化（文件存储）和读取
 *
 * 核心导出：
 * - Tokens：OAuth 令牌 Schema
 * - ClientInfo：OAuth 客户端信息 Schema
 * - McpAuth：认证服务（文件读写令牌、清除令牌）
 *
 * 架构位置：MCP 模块认证层，被 mcp/oauth-provider.ts 和 mcp/index.ts 使用。
 */
