/**
 * mcp/index - MCP 服务层：管理 Model Context Protocol 服务器连接和工具调用
 *
 * 功能概述：
 * - 管理 MCP 服务器的连接生命周期（stdio、SSE、Streamable HTTP）
 * - 提供工具列表、工具调用、资源访问功能
 * - 支持 OAuth 认证流程
 * - 管理服务器状态和连接池
 *
 * 核心导出：
 * - McpClient：MCP 客户端管理服务
 * - Tool：统一工具类型定义
 *
 * 架构位置：MCP 模块核心，提供与外部 MCP 服务器通信的统一接口。
 * 上游依赖：mcp/auth（认证）、mcp/oauth-provider（OAuth 提供者）、config/mcp（MCP 配置）
 */
