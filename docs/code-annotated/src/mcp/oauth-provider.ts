/**
 * mcp/oauth-provider - MCP OAuth 提供者：实现 MCP SDK 的 OAuthClientProvider 接口
 *
 * 功能概述：
 * - 实现 @modelcontextprotocol/sdk 的 OAuthClientProvider 接口
 * - 管理 OAuth 客户端元数据和令牌持久化
 * - 触发 OAuth 授权流程（打开浏览器、启动回调服务器）
 * - 支持自定义重定向 URI、客户端信息持久化
 *
 * 核心导出：
 * - createOauthProvider：创建 OAuth 提供者实例
 * - McpOAuthConfig：OAuth 配置接口
 *
 * 架构位置：MCP 模块认证层，被 mcp/index.ts 用于 MCP 服务器的 OAuth 认证。
 * 上游依赖：mcp/auth（令牌存储）、mcp/oauth-callback（回调服务器）
 */
