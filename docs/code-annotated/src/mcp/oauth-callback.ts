/**
 * mcp/oauth-callback - MCP OAuth 回调服务器：处理 OAuth 授权码回调
 *
 * 功能概述：
 ...
 *
 * 核心导出：
 * - startCallbackServer：启动回调服务器并等待授权码
 * - stopCallbackServer：停止回调服务器
 *
 * 架构位置：MCP 模块 OAuth 层，被 mcp/oauth-provider.ts 调用以完成 OAuth 流程。
 * 上游依赖：mcp/oauth-provider（回调端口和路径配置）
 */
