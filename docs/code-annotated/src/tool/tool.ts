/**
 * tool/tool - 工具系统核心类型定义
 *
 * 功能概述：
 * - 定义工具的基础类型：Context（执行上下文）、ExecuteResult（执行结果）
 * - 定义 Def/DefWithoutID：工具的 Schema 和执行函数接口
 * - 定义 Info：完整的工具元信息（id、description、parameters、jsonSchema）
 * - 定义 ToolPart 相关的类型映射（InferInput、InferMetadata）
 * - 提供工具执行结果编码/类型推断辅助类型
 *
 * 核心导出：
 * - Context：工具执行上下文（sessionID、messageID、agent、abort、ask 等）
 * - ExecuteResult：工具执行返回值（title、metadata、output、attachments）
 * - Def/DefWithoutID：工具定义接口
 * - Info：完整工具元信息类型
 * - InferInput/InferMetadata：从 Def 推断输入/元数据类型
 *
 * 架构位置：工具系统的基础层，被 tool/ 下所有具体工具实现依赖。
 * 上游依赖：session/message-v2（Part 类型）、permission（权限请求类型）
 * 下游消费：所有工具实现文件、tool/registry.ts、session/prompt.ts
 */
