/**
 * tool/json-schema - JSON Schema 生成工具
 *
 * 功能概述：
 * - 将 Effect Schema 转换为 JSON Schema 格式
 * - 供 AI SDK 提供者使用
 *
 * 核心导出：
 * - fromSchema：Schema 转 JSON Schema
 * - fromTool：工具定义转 JSON Schema
 *
 * 架构位置：工具系统实现层，被 tool/registry.ts 调用，为工具定义生成 JSON Schema
 */
