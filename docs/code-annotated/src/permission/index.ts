/**
 * permission/index - 权限管理服务：用户权限请求的评估、记录和管理
 *
 * 功能概述：
 * - 提供权限请求的评估（allow/deny/ask）
 * - 持久化权限决策（允许/拒绝规则）
 * - 管理权限决策的缓存和生命周期
 * - 集成总线事件发布权限变更通知
 *
 * 核心导出：
 * - Service：权限管理服务 Context 类
 * - layer：权限服务 Effect Layer
 * - Action：权限动作类型（allow/deny/ask）
 *
 * 架构位置：权限模块服务层，是 opencode 安全权限系统的核心。
 * 上游依赖：permission/evaluate（规则评估）、permission/schema（权限 ID）、storage/db（数据库）
 */
import { Bus } from "@/bus"
