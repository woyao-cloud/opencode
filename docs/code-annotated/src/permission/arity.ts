/**
 * permission/arity - Shell 命令前缀识别：通过命令前缀字典确定"人类可理解的命令"
 *
 * 功能概述：
 * - 通过最长匹配策略识别 shell 命令的"命令前缀"
 * - 内置大量常见命令的 arity 字典（git、npm、docker 等）
 * - 用于权限评估中的命令提取
 *
 * 核心导出：
 * - prefix：从 token 数组中提取命令前缀
 *
 * 架构位置：权限模块底层工具，被权限评估逻辑用于识别命令。
 */
