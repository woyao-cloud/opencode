/**
 * project/instance-store - 实例存储服务：管理项目实例的加载、缓存和销毁
 *
 * 功能概述：
 * - 加载和缓存项目实例（按目录路径）
 * - 管理实例的完整生命周期（创建、销毁、重新加载）
 * - 集成总线事件通知实例变更
 *
 * 核心导出：
 * - Service：实例存储服务 Context 类
 * - layer：实例存储 Effect Layer
 * - LoadInput：加载输入参数接口
 *
 * 架构位置：Project 模块存储层，管理所有活跃项目实例。
 * 上游依赖：project/bootstrap-service（引导服务）、effect/instance-ref（实例引用）
 */
