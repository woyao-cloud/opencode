/**
 * project/vcs - 版本控制系统服务：管理 Git 状态、差异和变更通知
 *
 * 功能概述：
 * - 获取工作目录的 Git 状态和差异
 * - 监听文件变更并生成差异补丁
 ...
 *
 * 核心导出：
 * - Service：VCS 服务 Context 类
 * - layer：VCS 服务 Effect Layer
 *
 * 架构位置：Project 模块服务层，提供版本控制集成。
 * 上游依赖：git/index（Git 服务）、file/watcher（文件监听器）
 */
