/**
 * shell/shell - Shell 服务：管理交互式 Shell 进程的创建和执行
 *
 * 功能概述：
 * - 检测系统可用 Shell（bash、zsh、fish、powershell 等）
 * - 创建和管理 Shell 子进程
 * - 支持登录 Shell、POSIX 兼容模式等
 *
 * 核心导出：
 * - Service：Shell 服务 Context 类
 * - layer：Shell 服务 Effect Layer
 *
 * 架构位置：Shell 模块服务层，为 opencode 提供交互式 Shell 能力。
 */
