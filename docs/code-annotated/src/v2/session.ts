/**
 * v2/session - V2 会话服务：新版会话管理，基于 Effect 和 V2 Schema
 *
 * 功能概述：
 * - 创建、查询和更新 V2 会话
 * - 管理会话消息
 * - 集成 SessionEvent 和 V2Schema
 *
 * 核心导出：
 * - Service：V2 会话服务 Context 类
 * - layer：V2 会话 Effect Layer
 *
 * 架构位置：V2 模块会话层，提供新版会话管理能力。
 * 上游依赖：session/session.sql（会话数据库表）、session/schema（会话 Schema）
 */
